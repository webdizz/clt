...
public class ExtensionGenerator extends Generator {
	...
	private static IconInfo[] createIconResources(TreeLogger logger, GeneratorContext context, JClassType userType,
		...
		for (int i = 0, n = icons.length; i < n; ++i) {
			...
			try {
				// Read the icon's byte data and decode it to determine the
				// size.
	...
	private static ExtensionArtifact getSpecification(TreeLogger logger, GeneratorContext context, JClassType userType)
		...
		logger.log(TreeLogger.ERROR,
				"You need a @Extension.Specification annotation on " + userType.getQualifiedSourceName()
